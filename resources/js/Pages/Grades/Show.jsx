import React from "react";
import DashboardLayout from "@/Layouts/DashboardLayout";
import Button from "@/Components/Button";
import { Head, usePage } from "@inertiajs/react";
import hasAnyPermission from "@/Utils/Permissions";

export default function Show() {
    const { grade } = usePage().props;

    const getGradeColor = (score) => {
        if (score >= 90) return 'bg-green-100 text-green-700';
        if (score >= 80) return 'bg-blue-100 text-blue-700';
        if (score >= 70) return 'bg-yellow-100 text-yellow-700';
        if (score >= 60) return 'bg-orange-100 text-orange-700';
        return 'bg-red-100 text-red-700';
    };

    const getGradeLetter = (score) => {
        if (score >= 90) return 'A';
        if (score >= 80) return 'B';
        if (score >= 70) return 'C';
        if (score >= 60) return 'D';
        return 'E';
    };

    const getStatusColor = (status) => {
        switch (status) {
            case 'draft':
                return 'bg-gray-100 text-gray-700';
            case 'published':
                return 'bg-green-100 text-green-700';
            case 'archived':
                return 'bg-yellow-100 text-yellow-700';
            default:
                return 'bg-gray-100 text-gray-700';
        }
    };

    const getStatusLabel = (status) => {
        switch (status) {
            case 'draft':
                return 'Draft';
            case 'published':
                return 'Dipublikasikan';
            case 'archived':
                return 'Diarsipkan';
            default:
                return status;
        }
    };

    const getAssessmentTypeLabel = (type) => {
        switch (type) {
            case 'task':
                return 'Tugas';
            case 'quiz':
                return 'Kuis';
            case 'exam':
                return 'Ujian';
            default:
                return type;
        }
    };

    const isPassing = grade.score >= 60;

    return (
        <DashboardLayout title={`Detail Nilai: ${grade.student?.name || 'Siswa'}`}>
            <Head title={`Detail Nilai: ${grade.student?.name || 'Siswa'}`} />

            <div className="space-y-6">
                <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                    <div className="h-1 w-full bg-gradient-to-r from-[#163d8f] via-[#2453b8] to-[#5b84d9]" />
                    <div className="border-b border-slate-200 bg-slate-50/70 px-6 py-5">
                        <div className="flex flex-wrap items-start justify-between gap-4">
                            <div>
                                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                                    Detail nilai siswa
                                </p>
                                <h2 className="mt-1 text-2xl font-semibold text-slate-900">
                                    {grade.student?.name || "Nama Siswa"}
                                </h2>
                                <p className="mt-1 text-sm text-slate-600">
                                    NIS: {grade.student?.nis || "-"} · {grade.class?.name || "Kelas"} ·{" "}
                                    {grade.subject?.name || "Mata Pelajaran"}
                                </p>
                            </div>
                            <div className="flex gap-2">
                                {hasAnyPermission(["grades edit"]) && (
                                    <Button
                                        type={"edit"}
                                        url={route("grades.edit", grade.id)}
                                    />
                                )}
                                {hasAnyPermission(["grades delete"]) && (
                                    <Button
                                        type={"delete"}
                                        url={route("grades.destroy", grade.id)}
                                    />
                                )}
                            </div>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 gap-4 px-6 py-5 md:grid-cols-3">
                        <div className="rounded-md border border-slate-200 bg-slate-50/80 p-4 text-center">
                            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Nilai angka
                            </p>
                            <p className="mt-1 text-3xl font-semibold text-slate-900">
                                {grade.score}%
                            </p>
                        </div>
                        <div className="rounded-md border border-slate-200 bg-slate-50/80 p-4 text-center">
                            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Grade huruf
                            </p>
                            <p className={`mt-1 text-3xl font-semibold ${getGradeColor(grade.score)}`}>
                                {getGradeLetter(grade.score)}
                            </p>
                        </div>
                        <div className="rounded-md border border-slate-200 bg-slate-50/80 p-4 text-center">
                            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Status kelulusan
                            </p>
                            <p className={`mt-1 text-xl font-semibold ${isPassing ? "text-emerald-700" : "text-rose-700"}`}>
                                {isPassing ? "LULUS" : "TIDAK LULUS"}
                            </p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 gap-4 border-t border-slate-100 px-6 py-5 md:grid-cols-2 lg:grid-cols-4">
                        <div>
                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Tipe Penilaian
                            </label>
                            <p className="mt-1 text-sm font-medium text-slate-900">
                                {getAssessmentTypeLabel(grade.assessment_type)}
                            </p>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Nama Penilaian
                            </label>
                            <p className="mt-1 text-sm font-medium text-slate-900">
                                {grade.assessment?.title || '-'}
                            </p>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Status
                            </label>
                            <span className={`mt-1 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${getStatusColor(grade.status)}`}>
                                {getStatusLabel(grade.status)}
                            </span>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Tanggal Input
                            </label>
                            <p className="mt-1 text-sm font-medium text-slate-900">
                                {new Date(grade.created_at).toLocaleDateString('id-ID')}
                            </p>
                        </div>
                    </div>

                    {grade.feedback && (
                        <div className="border-t border-slate-100 px-6 py-5">
                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">
                                Feedback Guru
                            </label>
                            <div className="rounded-md border border-[#d9e5f8] bg-[#eef4ff] p-4">
                                <div className="prose prose-sm max-w-none text-slate-800">
                                    {grade.feedback.split('\n').map((line, i) => (
                                        <p key={i} className="mb-2">{line}</p>
                                    ))}
                                </div>
                            </div>
                        </div>
                    )}

                </div>

                {grade.assessment && (
                    <div className="rounded-lg border border-slate-200 bg-white p-6">
                            <h3 className="text-lg font-semibold text-slate-900 mb-4">
                                Detail Penilaian
                            </h3>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                        Judul
                                    </label>
                                    <p className="mt-1 text-sm text-slate-900">
                                        {grade.assessment.title}
                                    </p>
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                        Deskripsi
                                    </label>
                                    <p className="mt-1 text-sm text-slate-900">
                                        {grade.assessment.description || 'Tidak ada deskripsi'}
                                    </p>
                                </div>
                                {grade.assessment_type === 'task' && (
                                    <>
                                        <div>
                                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                                Deadline
                                            </label>
                                            <p className="mt-1 text-sm text-slate-900">
                                                {grade.assessment.deadline ? new Date(grade.assessment.deadline).toLocaleString('id-ID') : '-'}
                                            </p>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                                Prioritas
                                            </label>
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                grade.assessment.priority === 'high' ? 'bg-red-100 text-red-700' :
                                                grade.assessment.priority === 'medium' ? 'bg-yellow-100 text-yellow-700' :
                                                'bg-green-100 text-green-700'
                                            }`}>
                                                {grade.assessment.priority === 'high' ? 'Tinggi' :
                                                 grade.assessment.priority === 'medium' ? 'Sedang' : 'Rendah'}
                                            </span>
                                        </div>
                                    </>
                                )}
                                {grade.assessment_type === 'quiz' && (
                                    <>
                                        <div>
                                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                                Jumlah Soal
                                            </label>
                                            <p className="mt-1 text-sm text-slate-900">
                                                {grade.assessment.total_questions || '-'} soal
                                            </p>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                                Durasi
                                            </label>
                                            <p className="mt-1 text-sm text-slate-900">
                                                {grade.assessment.duration ? `${grade.assessment.duration} menit` : 'Tidak terbatas'}
                                            </p>
                                        </div>
                                    </>
                                )}
                                {grade.assessment_type === 'exam' && (
                                    <>
                                        <div>
                                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                                Jadwal Ujian
                                            </label>
                                            <p className="mt-1 text-sm text-slate-900">
                                                {new Date(grade.assessment.scheduled_date).toLocaleDateString('id-ID')}
                                            </p>
                                            <p className="text-xs text-slate-500">
                                                {grade.assessment.start_time} - {grade.assessment.end_time}
                                            </p>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500">
                                                Tipe Ujian
                                            </label>
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                grade.assessment.type === 'midterm' ? 'bg-blue-100 text-blue-700' :
                                                grade.assessment.type === 'final' ? 'bg-red-100 text-red-700' :
                                                grade.assessment.type === 'quiz' ? 'bg-green-100 text-green-700' :
                                                'bg-purple-100 text-purple-700'
                                            }`}>
                                                {grade.assessment.type === 'midterm' ? 'UTS' :
                                                 grade.assessment.type === 'final' ? 'UAS' :
                                                 grade.assessment.type === 'quiz' ? 'Kuis' : 'Latihan'}
                                            </span>
                                        </div>
                                    </>
                                )}
                            </div>
                    </div>
                )}

                <div className="rounded-lg border border-slate-200 bg-white p-6">
                        <h3 className="text-lg font-semibold text-slate-900 mb-4">
                            Riwayat Performa Siswa
                        </h3>

                        <div className="text-sm text-slate-600 mb-4">
                            Statistik performa {grade.student?.name} di mata pelajaran {grade.subject?.name}
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="rounded-md border border-slate-200 bg-slate-50/80 p-4 text-center">
                                <div className="text-2xl font-semibold text-[#163d8f]">
                                    {grade.student?.grades?.filter(g => g.subject_id === grade.subject_id).length || 0}
                                </div>
                                <div className="text-sm text-slate-600">Total Penilaian</div>
                            </div>
                            <div className="rounded-md border border-emerald-200 bg-emerald-50/70 p-4 text-center">
                                <div className="text-2xl font-semibold text-emerald-700">
                                    {grade.student?.grades?.filter(g => g.subject_id === grade.subject_id && g.score >= 90).length || 0}
                                </div>
                                <div className="text-sm text-slate-600">Grade A</div>
                            </div>
                            <div className="rounded-md border border-sky-200 bg-sky-50/70 p-4 text-center">
                                <div className="text-2xl font-semibold text-sky-700">
                                    {grade.student?.grades?.filter(g => g.subject_id === grade.subject_id).length > 0
                                        ? Math.round(grade.student.grades.filter(g => g.subject_id === grade.subject_id).reduce((sum, g) => sum + g.score, 0) / grade.student.grades.filter(g => g.subject_id === grade.subject_id).length)
                                        : 0}%
                                </div>
                                <div className="text-sm text-slate-600">Rata-rata</div>
                            </div>
                            <div className="rounded-md border border-violet-200 bg-violet-50/70 p-4 text-center">
                                <div className="text-2xl font-semibold text-violet-700">
                                    {grade.student?.grades?.filter(g => g.subject_id === grade.subject_id && g.score >= 60).length || 0}
                                </div>
                                <div className="text-sm text-slate-600">Lulus</div>
                            </div>
                        </div>
                </div>
            </div>
        </DashboardLayout>
    );
}